...
EMBEDDING_MODEL = "all-MiniLM-L6-v2" # alternative: 
EMBEDDING_LENGTH = 384  # Embedding vector length for all-MiniLM-L6-v2 model (not 512)
FOLDER_PATH = "./data/output/final_merged"  # Folder containing .txt files
db_type = f"chroma_db_{CHUNK_STRATEGY}_{EMBEDDING_MODEL.replace('/', '_')}_{EMBEDDING_LENGTH}"
...
if __name__ == "__main__":
    ...
    if embedding_result is not None and 'embeddings' in embedding_result:
        ...
        if embeddings_data is not None and len(embeddings_data) > 0:
            ...
            print(f"First 10 values of embedding: {embedding_vector[:10]}")
