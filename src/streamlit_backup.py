...
# Note: Models are stored in st.session_state, not global variables
# because global variables reset on every Streamlit rerun

def load_llm_model(model_name: str):
    """Lazy load LLM model only when needed."""
    
    # Initialize model storage in session state if not exists
...
if 'skip_auto_load' not in st.session_state:
...
def clear_all_llm_models():
    ...
    # Delete model objects from session state
...
with st.sidebar:
    ...
    if st.session_state.rag_system:
        ...
        top_k = st.slider("Number of results:", 10, 30, 25, key="search_top_k")
    else:
    ...
    if st.button("🔄 Restart System", type="secondary", use_container_width=True, help="Clear all data and restart from scratch"):
        # Clear all LLM models from VRAM first
        clear_all_llm_models()
        
        # Clear all session state (this removes the model references permanently)
        for key in list(st.session_state.keys()):
            del st.session_state[key]
        
        # Set flag to skip auto-loading on next rerun
