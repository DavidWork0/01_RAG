...
try:
    import neptune
    print("✅ Neptune package installed")
...
if api_token:
    print(f"✅ NEPTUNE_API_TOKEN is set ({api_token[:10]}...)")
else:
    print("❌ NEPTUNE_API_TOKEN not set")
    print("   Set with: $env:NEPTUNE_API_TOKEN='your_token'")

if project:
    print(f"✅ NEPTUNE_PROJECT is set ({project})")
else:
    print("❌ NEPTUNE_PROJECT not set")
...
try:
    ...
    print("✅ Base HyperparameterTuner imported")
except ImportError as e:
    print(f"❌ Failed to import HyperparameterTuner: {e}")

try:
    from hyperparameter_tuner_neptune import NeptuneHyperparameterTuner
    print("✅ NeptuneHyperparameterTuner imported")
except ImportError as e:
    print(f"❌ Failed to import NeptuneHyperparameterTuner: {e}")
...
if chunking_script.exists():
    print(f"✅ Chunking script exists: {chunking_script}")
else:
    print(f"❌ Chunking script not found: {chunking_script}")
...
if not NEPTUNE_AVAILABLE:
    print("⚠️  Neptune package is not installed")
    print("   Run: pip install neptune")
    all_good = False

if not api_token or not project:
    print("⚠️  Neptune credentials not configured")
...
if all_good:
    print("\n✅ All checks passed! You're ready to run hyperparameter tuning.")
    print("\nNext steps:")
    ...
    print("\n⚠️  Some issues found. Please fix them before running tuning.")
